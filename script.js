...
const ICONS = {
    ...
    // Aimant en fer à cheval, branches vers le HAUT : contour extérieur, contour
...
// UNIQUEMENT sa propre identité (fondamentale/qualité/basse/voicing) ; la DURÉE et le verrou guitare
// (voir toggleGuitarLock) restent ceux du diagramme réellement affiché, verrou compris — celui-ci
// porte alors sur CE substitut plutôt que sur l'accord réellement joué, exactement comme s'il
// s'agissait d'un accord normal (retour utilisateur, point 2 : "même principe qu'actuellement, avec
// plusieurs types de jeu possibles, et fixation via le cadenas"). Jamais de notes libres (extraNotes) :
// ce sont des embellissements du voicing PIANO/séquenceur, sans rapport avec un substitut de guitare.
function guitarChordFor(chord, override) {
    if (!override) return chord;
    const c = new Chord(override.root, override.quality, chord.beats, override.inversion, override.drop, override.octave, override.bass, chord.guitarLock, []);
    // Repère pour renderGuitarOverrideRow (voir ensureGuitarDiagram) : distingue un Chord synthétique
    // de substitution d'un Chord normal SANS avoir à faire circuler l'info séparément partout où ce
    // même Chord voyage ensuite (schedulePlayback, ses rappels par croche...).
    c._guitarOverrideInfo = override;
    return c;
}
...
class HarmoHubApp {
    constructor() {
        ...
        // Durée « collante » des nouvelles notes (voir seqNewNoteLen) : longueur, en doubles croches,
        // de la dernière note réellement dessinée ou étirée. null = pas encore de référence, on pose
        // alors une croche. Volontairement NON persistée : une nouvelle session repart d'une durée
        // prévisible plutôt que d'une longueur invisible héritée d'hier.
        ...
        this.settingsOpen = false; // fenêtre Paramètres (Son, Affichage, Mes morceaux)
        this.filesOpen = false; // fenêtre « Mes morceaux » seule, ouverte depuis le module Morceau
    ...
    setupEventListeners() {
        ...
        document.addEventListener('click', (e) => {
            ...
            // ONZIÈME, ET C'EST CELLE QUI EXPLIQUE LE « DES FOIS » : le TRANSPORT (Lecture/Boucle/Stop).
            // placeGlobalTransport le déménage selon la mise en page — dans .col-left quand le panneau
            // de gauche est ouvert, dans .col-right quand il est replié. Ouvert, il était couvert par
            // .col-left et tout allait bien ; REPLIÉ, il tombait hors de toute zone d'édition et
            // cliquer Lecture sortait de la modification, d'où un séquenceur qui « saute et redevient
            // sommaire »... une fois sur deux, selon l'état du panneau. C'est exactement le
            // « des fois ça saute » signalé, et la raison pour laquelle deux de mes propres sondes se
            // contredisaient : l'une avait replié le panneau au cas précédent.
            // Visé par #footer-dock ET .transport — son accueil et lui-même —, donc indépendamment de
            // l'endroit où il est posé. C'est la leçon des dix fois précédentes, appliquée d'avance.
            // NEUVIÈME et DIXIÈME : #toggle-sidebar (replier le panneau de gauche) et
            // #toggle-voice-leading (ouvrir la conduite de voix). Deux bascules de VUE, comme les
            // échelles ci-dessous : regarder son travail autrement n'est pas le quitter. Le panneau de
            // gauche héberge justement le module d'édition — le replier en cours de modification pour
            // gagner de la place est un geste normal, qui perdait tout.
            // SEPTIÈME et HUITIÈME fois, trouvées d'un coup en balayant TOUS les boutons visibles
            // plutôt qu'en attendant le prochain signalement : les six groupes d'échelle
            // (.zoom-axis-group — H/V de la grille comme du séquenceur) faisaient sortir de l'édition.
            // Régler une ÉCHELLE D'AFFICHAGE n'est jamais quitter son travail : c'est le regarder
            // autrement. Visés par leur classe commune, donc valables où qu'ils soient placés.
            // SIXIÈME fois : .seq-dock-panel. #arp-sequencer ne couvre que le séquenceur lui-même, pas
            // le VOLET qui l'accueille sous la grille — or ce volet a sa propre poignée de hauteur
            // (#seq-dock-resize), en dehors. Mesuré : tirer cette poignée en cours de modification
            // faisait retomber editingIndex à null, et le rendu suivant repassait le séquenceur en vue
            // sommaire, puisque la vue continue exige editingIndex != null (voir `continuous` dans
            // renderSequencer). C'est le « des fois le séquenceur saute et redevient un séquenceur
            // sommaire » signalé par l'utilisateur. Le volet entier est visé, pas la seule poignée :
            // tout ce que le volet ajoutera autour du séquenceur relève du même travail en cours.
    ...
    ensureGuitarDiagram(chord, useLiveLock = true) {
        ...
        // Bandeau au-dessus du diagramme (voir renderGuitarOverrideRow, point 3 du retour utilisateur) :
        // en édition/Ajout, suit l'état LIVE (this.guitarOverride) ; en lecture seule, le repère posé
        // sur CE Chord précis par guitarChordFor (chord._guitarOverrideInfo) — jamais this.guitarOverride
        // ici, qui ne concerne que le panneau live, sans rapport avec un accord juste écouté/joué.
        this.renderGuitarOverrideRow(useLiveLock ? this.guitarOverride : (chord._guitarOverrideInfo || null));
    }
    ...
    // volontairement les deux instruments sur certaines parties.

    // Ouvre la saisie au clavier du nom de substitution (point 1 : "changer le nom de l'accord
    // manuellement au clavier uniquement") — même principe que startInlineChordSymbolEdit (édition
    ...
    // voir toggleGuitarLock pour le même principe) — pour préremplir le champ de saisie sans passer
    // par this.guitarOverride, qui ne concerne que le panneau live (édition/Ajout).
    _previewGuitarOverride() {
        const { section, index } = this.guitarPreviewPos;
        const data = loadProgressionSections()[section]?.chords[index];
        return data ? (data.guitarOverride || null) : null;
    }

    // Écrit le nouvel accord de substitution (ou le retire si `override` est null) — cible le MÊME
    // accord que le cadenas (voir toggleGuitarLock, même logique de ciblage reprise à l'identique) :
    // simple aperçu (this.guitarPreviewPos) sans édition ouverte -> directement dans les données de
    // CET accord précis ; sinon l'état LIVE du panneau (édition en cours ou nouvel accord pas encore
    // ajouté, capturé au moment d'Ajouter/Modifier via buildLiveChordData). Un ancien verrou guitare
    // (voir toggleGuitarLock) ne correspond plus forcément à ce qui est désormais affiché (l'accord
    // réellement joué, OU le nouveau substitut) : reparti à zéro, comme changeChordOctave/
    // startInlineChordSymbolEdit pour les autres réglages qui changent l'identité affichée.
    applyGuitarOverride(override) {
        if (this.guitarPreviewPos) {
            const { section, index } = this.guitarPreviewPos;
            const sections = loadProgressionSections();
            const data = sections[section] && sections[section].chords[index];
        ...
        this.guitarKey = null;
        this.commitLiveEdit(false); // n'affecte pas le symbole affiché dans la case (mode Modification) ; sans effet en Ajout
        this.refreshPreview();
    }

    clearGuitarOverride() {
        this.applyGuitarOverride(null);
    }

    // Reflète l'état actuel de l'accord de substitution au-dessus du diagramme guitare (voir
    ...
    clearGuitarViz() {
        ...
        // Bandeau de substitution (voir renderGuitarOverrideRow) : vidé avec le reste, le prochain
        // ensureGuitarDiagram le repeuple s'il y a lieu — sinon il flotterait un instant au-dessus
        // d'un diagramme vide.
        const overrideRow = document.getElementById('guitar-override-row');
        if (overrideRow) { overrideRow.hidden = true; overrideRow.innerHTML = ''; }
    }
            ...
            eventIds.push(Tone.Transport.schedule((t) => {
                Tone.Draw.schedule(() => {
                    ...
                    try {
                        ...
                        // posés par un diagramme périmé (retour utilisateur : « le cadenas se désactive »,
                        // et même bug observé pour le substitut lors des tests de cette fonctionnalité).
                        ...
                        if (!trackPlayhead && gridPos) {
                            ...
                            displayChord = guitarChordFor(chord, freshData ? (freshData.guitarOverride || null) : null);
                        } else if (trackPlayhead) {
    ...
    // Aimantation FIXE à la double croche : une case de la grille EST une double croche
    // (SEQ_STEPS_PER_BEAT = 4), donc « se caler sur la case » ne restreint rien — c'est la précision
    // maximale que la vue permet. Le sélecteur qui proposait 1/4 et 1/8 a été retiré (retour
    // utilisateur) : il fallait le régler avant de dessiner, et l'oublier posait des notes au mauvais
    // endroit. Le penchant pour la croche vit dans snapSeqBoundary, pas ici.
    seqSnap() {
        return 1;
    }

    // Léger AIMANT sur les croches (voir snapSeqBoundary/onSeqPointerMove) : quand la position tombe
    // pile sur un début de croche, on l'y garde ; sinon on reste à la double croche visée. Concrètement
    // la croche gagne un peu de « poids » sans jamais empêcher d'atteindre une double croche — retour
    // utilisateur : « si tu penses qu'il faut renforcer légèrement l'accrochage sur les croches par
    // rapport à l'accrochage en double croche, tu peux mettre cela en place ». 0.3 : on ne bascule sur
    // la croche que dans le tiers de case qui la borde, la majorité de la case restant à elle-même.
    _aimantCroche(posFractionnaire) {
        const PAS_CROCHE = 2; // 2 doubles croches = 1 croche
        const croche = Math.round(posFractionnaire / PAS_CROCHE) * PAS_CROCHE;
        return Math.abs(posFractionnaire - croche) <= 0.3 ? croche : Math.round(posFractionnaire);
    }
    ...
    snapSeqBoundary(pos) {
        return this._aimantCroche(pos);
    }

    // Longueur, en doubles croches, d'une note créée par un simple clic/tap : celle de la dernière
    // note réellement dessinée ou étirée (voir this.seqStickyLen — même principe que GarageBand/Logic,
    // qui réutilisent la dernière durée éditée). À défaut, une CROCHE : c'est la durée la plus
    // courante, et l'ancien repli (le pas d'aimantation) n'existe plus depuis que celui-ci est fixe.
    seqNewNoteLen() {
        return Math.max(1, this.seqStickyLen || 2);
    }
    ...
    toggleSequencer(mode = 'compact') {
        ...
        // barre de grille qui vient de l'ouvrir : mesuré à ~560px sous un écran de 844px, donc
        // totalement invisible. Le bouton paraissait ne RIEN faire (retour utilisateur : « le
        // séquenceur ne veut pas s'ouvrir avec le bouton au-dessus de la grille »). On l'amène donc à
        // l'écran s'il n'y est pas déjà — sans effet sur ordinateur, où il est ancré juste sous la
        // grille et déjà visible.
        if (this.seqOpen) this.defilerVersSequenceur();
    }

    // Amène le séquenceur dans le champ de vision s'il en est sorti. Volontairement conditionnel :
    // faire défiler alors qu'il est déjà visible déplacerait la page sous les doigts sans raison.
    defilerVersSequenceur() {
    ...
    hauteurVoletSequenceur() {
        ...
        if (grille && hote && !this.seqDockHeight) {
            ...
            // Une ligne par demi-ton affiché, à 14px : le contenu ne dépend plus d'aucune échelle
            // depuis que la hauteur de barre est fixe.
            const nbLignes = grille.querySelectorAll('.seq-label').length;
    ...
    renderSequencer() {
        ...
        // Réservée à la VUE AGRANDIE du séquenceur (voir openSeqZoom/#seq-zoom). Le petit séquenceur
        // du panneau Accord garde son affichage d'origine : la seule durée de l'accord, et uniquement
        // les notes réellement jouées — retour utilisateur : « ça n'était pas le but de ce séquenceur,
        // je voulais le garder comme avant : pas de séquenceur continu mais seulement durée de
        // l'accord, afficher uniquement les notes jouées... il était plus précis pour les petites
        ...
        // Tout ce qui n'est PAS la vue agrandie a droit au réglage d'échelle horizontale dédié
        // (kind 'seqInline'/showInlineSeqZoom plus bas) ; la vue agrandie a le sien (seqZoomLevelX).
        // La vue continue vivant exclusivement dans cette vue agrandie, tester seqZoomOpen suffit.
        ...
        // avec elle. Masqué à l'arrêt, comme elle.
        if (continuous) html += `<div class="seq-ruler-head" hidden style="grid-row:${beatRow}; grid-column: 2 / span 1;"></div>`;
        ...
        // #seq-play ne priorise une plage à boucler (voir plus bas) qu'en VUE AGRANDIE : là, le
        // séquenceur montre toute la grille, on navigue dans son contexte entier, la plage a donc un
        ...
        html += `<div class="seq-presets">
            ...
            <!-- Le sélecteur d'aimantation (1/4, 1/8, 1/16) vivait ici. Retiré (retour utilisateur :
                 ...
                 pour un choix que la grille rend déjà évident — une case EST une double croche. Voir
                 snapSeqBoundary : l'aimantation est désormais fixe, avec un léger penchant pour la
                 croche quand on en passe tout près. -->
            ${showInlineSeqZoom ? `
    ...
    updateSeqPlayhead(step) {
        ...
        if (!grid || !ph) return;
        // Repère jumeau dans la règle de mesure (voir .seq-ruler-head) : même position, même
        // visibilité — piloté ici plutôt qu'ailleurs, sinon les deux finiraient par se contredire.
        const tete = host.querySelector('.seq-ruler-head');
        const cacher = () => { ph.style.display = 'none'; if (tete) tete.hidden = true; };
    ...
    async playSavedChord(section, index, play = true) {
        ...
        // édition (voir editChord), donc c'est la SEULE façon pour toggleGuitarLock/applyGuitarOverride
        // de savoir où écrire si on clique le cadenas/le crayon directement depuis un simple aperçu,
        // sans avoir double-cliqué pour ouvrir l'édition complète.
        this.guitarPreviewPos = { section, index };
        this.ensureGuitarDiagram(guitarChord, false);
        ...
        Tone.Transport.schedule((t) => {
            Tone.Draw.schedule(() => {
                try {
                    // `chord`/`guitarChord` sont figés au lancement de cette lecture : relit le verrou ET
                    // le substitut depuis les données à cet instant, au cas où ils auraient changé
                    // entre-temps (voir le même principe dans schedulePlayback ci-dessus).
                    const freshData = loadProgressionSections()[section]?.chords[index];
                    let freshGuitarChord = guitarChord;
                    if (freshData) {
                        const freshChord = new Chord(chord.root, chord.quality, chord.beats, chord.inversion, chord.drop, chord.octave, chord.bass, freshData.guitarLock || null, chord.extraNotes);
                        freshGuitarChord = guitarChordFor(freshChord, freshData.guitarOverride);
                    }
                    this.ensurePianoWindow(midis); this.updateViz(midis, roleMap); this.ensureGuitarDiagram(freshGuitarChord, false);
                } catch (e) {
